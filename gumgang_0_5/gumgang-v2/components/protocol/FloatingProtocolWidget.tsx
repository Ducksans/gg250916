    ...
    const connectSSE = () => {
      try {
        const eventSource = new EventSource(
          "http://localhost:8001/api/protocol/stream",
        );
  ...
  const updateProtocolState = (data: any) => {
    setProtocolState((prev) => ({
      ...
      currentTask: data.currentTask
        ? {
            ...data.currentTask,
            startTime: data.currentTask.startTime
              ? new Date(data.currentTask.startTime)
              : undefined,
          }
        : null,
    }));
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1">
          <Shield
            className={`w-4 h-4 ${getTrustScoreColor(protocolState.trustScore)}`}
          />
          <span
            className={`font-bold ${getTrustScoreColor(protocolState.trustScore)}`}
          >
          ...
          <>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-300">
                {protocolState.currentTask.category}.
                {protocolState.currentTask.subcategory}
              </span>
    ...
    <div className="bg-gray-900 text-white rounded-lg shadow-2xl w-96 max-h-[600px] overflow-hidden flex flex-col">
      ...
      <div className="p-4 border-b border-gray-700">
        <div className="flex items-center justify-between mb-2">
          ...
          <span
            className={`font-bold text-lg ${getTrustScoreColor(protocolState.trustScore)}`}
          >
            {protocolState.trustScore}%
          </span>
        </div>
      ...
      {protocolState.currentTask && (
        <div className="p-4 border-b border-gray-700">
          <div className="flex items-center justify-between mb-2">
            ...
            <span
              className={`text-sm ${getRiskColor(protocolState.currentTask.risk)}`}
            >
              {getRiskEmoji(protocolState.currentTask.risk)}{" "}
              {protocolState.currentTask.risk}
            </span>
          </div>

          <div className="bg-gray-800 rounded p-2 mb-3">
            <div className="text-xs text-gray-400 mb-1">
              {protocolState.currentTask.id}
            </div>
          ...
          <div className="mb-3">
            <div className="flex justify-between text-xs text-gray-400 mb-1">
              <span>
                {protocolState.currentTask.currentAction || "작업 진행 중..."}
              </span>
          ...
          <div className="flex items-center gap-3">
            ...
            <label className="flex items-center gap-1 text-xs">
              <input
                ...
                onChange={(e) =>
                  sendCommand("toggleAutoSave", { enabled: e.target.checked })
                }
                className="w-3 h-3"
              />
              자동저장
