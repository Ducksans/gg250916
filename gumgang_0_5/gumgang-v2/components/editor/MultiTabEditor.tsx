...
export const MultiTabEditor: React.FC<MultiTabEditorProps> = ({
  ...
  const handleWebContentChange = useCallback(
  ...
  // Check if Tauri is available (hardened)
  // Require window.__TAURI__.core.invoke to exist; otherwise treat as web mode
  useEffect(() => {
    const ok = (() => {
      if (typeof window === "undefined") return false;
      const anyWin = window as any;
      const t = anyWin && anyWin.__TAURI__;
      return !!(
        t &&
        typeof t === "object" &&
        t.core &&
        typeof t.core.invoke === "function"
      );
    })();
    setIsTauriAvailable(ok);
  }, []);

  const {
    // isAvailable, // Removed: unused variable
    readFile,
    writeFile,
    // readDirectory, // Removed: unused variable
    openFileDialog,
    saveFileDialog,
    // isLoading,
    // error,
        ...
        {activeTab ? (
          isTauriAvailable ? (
            <FileEditor
              ...
              height={height}
              onSave={(_path, _content) => saveFile(activeTab.id)}
            />
          ) : (
            <div className="h-full" style={{ height, minHeight: height }}>
              <MonacoEditor
                key={activeTab.id}
