...
# 상위 디렉토리 모듈 임포트
import sys
# sys.path.append removed - using relative imports
from ..core.memory.temporal import (
    MemoryTrace, MemoryType, MemoryPriority,
    get_temporal_memory_system
)
from ..core.cognition.meta import get_metacognitive_system
from .dream import get_dream_system
from .creative import get_creative_association_engine
...
class EmotionalEmpathySystem:
    ...
    def inject_dependencies(self, deps: Dict[str, Any]):
        ...
        Args:
            deps: 주입할 의존성 딕셔너리
                - temporal_memory: 시간적 메모리 시스템
                - meta_cognitive: 메타인지 시스템
                - creative_engine: 창의 연상 엔진
                - dream_system: 꿈 시스템
        """
        if 'temporal_memory' in deps:
            self.temporal_memory = deps['temporal_memory']
        if 'meta_cognitive' in deps:
            self.meta_cognitive = deps['meta_cognitive']
        if 'creative_engine' in deps:
            self.creative_engine = deps['creative_engine']
        if 'dream_system' in deps:
            self.dream_system = deps['dream_system']

        # 의존성이 주입되면 초기화된 것으로 표시
        if self.temporal_memory and self.meta_cognitive:
            self._initialized = True
            logger.info("✅ 의존성 주입 완료: 감정 공감 시스템")
